...
export class OrderController {
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: '정렬 조건 (예: col0.desc)',
  })
  @ApiQuery({
    name: 'dateFrom',
    required: false,
    type: String,
    description: '시작 날짜 (ISO 문자열)',
  })
  @ApiQuery({
    name: 'dateTo',
    required: false,
    type: String,
    description: '종료 날짜 (ISO 문자열)',
  })
  @ApiQuery({
    name: 'dateField',
    required: false,
    type: String,
    description: '날짜 필드명 (기본값: createdAt)',
  })
  @ApiQuery({
    name: 'status',
    required: false,
    type: String,
    description: '진행상태 필터 (콤마로 구분)',
  })
  @ApiQuery({
    name: 'customer',
    required: false,
    type: String,
    description: '발주처 필터 (콤마로 구분)',
  })
  @ApiQuery({
    name: 'productName',
    required: false,
    type: String,
    description: '제품명 필터 (콤마로 구분)',
  })
  @ApiQuery({
    name: 'partName',
    required: false,
    type: String,
    description: '부속명 필터 (콤마로 구분)',
  })
  async getOrders(
  ...
  @ApiOperation({
    ...
    description:
      '발주번호 배열을 받아 해당하는 주문 정보들을 조회합니다. 발주번호 형식 검증을 포함합니다.',
  })
  @ApiQuery({
    name: 'orderNumbers',
    required: true,
    type: String,
    description:
      '콤마로 구분된 발주번호 목록 (예: T00000-1,T00000-2). 영문자로 시작하고 숫자와 하이픈을 포함하는 형식',
  ...
  async searchOrders(@Query('orderNumbers') orderNumbersParam: string): Promise<
    CommonApiResponse<{
  ...
  > {
    ...
    const { missing: missingOrderNumbers, invalid: invalidOrderNumbers } =
      existenceCheck;
  ...
  @ApiOperation({
    summary: '필터 옵션 조회',
    description: '테이블 필터링에 사용할 수 있는 옵션들을 조회합니다.',
  })
  @ApiResponse({
    status: 200,
  ...
  > {
    const filterOptions = await this.orderService.getFilterOptions();
