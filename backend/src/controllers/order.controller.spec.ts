...
describe('OrderController', () => {
  ...
  describe('searchOrders', () => {
    it('should throw ValidationException for empty orderNumbers parameter', async () => {
      await expect(controller.searchOrders('')).rejects.toThrow(
        ValidationException,
      );
      await expect(controller.searchOrders(null as any)).rejects.toThrow(
        ValidationException,
      );
      await expect(controller.searchOrders(undefined as any)).rejects.toThrow(
        ValidationException,
      );
    });

    it('should throw ValidationException for too many order numbers', async () => {
      const tooManyOrderNumbers = Array.from(
        { length: 51 },
        (_, i) => `T${String(i).padStart(5, '0')}-1`,
      ).join(',');

      await expect(
        controller.searchOrders(tooManyOrderNumbers),
    ...
    it('should return successful response with order data', async () => {
      ...
      mockOrderService.checkOrderNumbersExistence.mockResolvedValue(
        mockExistenceCheck,
      );
      ...
      expect(mockOrderService.searchByOrderNumbers).toHaveBeenCalledWith([
        'T00000-1',
        'T00000-2',
      ]);
      expect(mockOrderService.checkOrderNumbersExistence).toHaveBeenCalledWith([
        'T00000-1',
        'T00000-2',
      ]);
    });
  ...
  describe('getOrderByNumber', () => {
    it('should throw ValidationException for empty orderNumber parameter', async () => {
      await expect(controller.getOrderByNumber('')).rejects.toThrow(
        ValidationException,
      );
      await expect(controller.getOrderByNumber(null as any)).rejects.toThrow(
        ValidationException,
      );
      await expect(
        controller.getOrderByNumber(undefined as any),
      ).rejects.toThrow(ValidationException);
    });
