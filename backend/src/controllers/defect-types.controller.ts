import { Controller, Get, Post, Put, Delete, Body, Param, UseGuards, HttpException, HttpStatus, Request, ParseIntPipe } from '@nestjs/common';
...
export class DefectTypesController {
  ...
  async getDefectTypes(): Promise<CommonApiResponse<any[]>> {
    ...
    return CommonApiResponse.success(defectTypes, '불량 유형 목록을 성공적으로 조회했습니다.');
  }
  ...
  async getDefectTypeById(@Param('id', ParseIntPipe) id: number): Promise<CommonApiResponse<any>> {
    const defectType = await this.defectTypesService.getDefectTypeById(id);
    if (!defectType) {
      throw new HttpException('불량 유형을 찾을 수 없습니다', HttpStatus.NOT_FOUND);
    }
    return CommonApiResponse.success(defectType, '불량 유형을 성공적으로 조회했습니다.');
  }
  ...
  async createDefectType(@Body() createDefectTypeDto: CreateDefectTypeDto, @Request() req: any): Promise<CommonApiResponse<any>> {
    ...
    try {
      const defectType = await this.defectTypesService.createDefectType(createDefectTypeDto);
      return CommonApiResponse.success(defectType, '불량 유형이 성공적으로 생성되었습니다.');
    } catch (error: any) {
      if (error.code === '23505') { // Unique constraint violation
        throw new HttpException('이미 존재하는 불량 유형입니다', HttpStatus.BAD_REQUEST);
      }
      throw error;
    }
  }
  ...
  @ApiOperation({ 
    summary: '불량 유형 수정',
    description: '기존 불량 유형의 정보를 수정합니다. 관리자 권한이 필요합니다.'
  })
  @ApiParam({ 
    name: 'id', 
    description: '불량 유형 ID',
  ...
  async updateDefectType(
    ...
    try {
      const updatedDefectType = await this.defectTypesService.updateDefectType(id, updateDefectTypeDto);
      if (!updatedDefectType) {
        throw new HttpException('불량 유형을 찾을 수 없습니다', HttpStatus.NOT_FOUND);
      }
      return CommonApiResponse.success(updatedDefectType, '불량 유형이 성공적으로 수정되었습니다.');
    } catch (error: any) {
      if (error.code === '23505') { // Unique constraint violation
        throw new HttpException('이미 존재하는 불량 유형입니다', HttpStatus.BAD_REQUEST);
      }
  ...
  async deleteDefectType(@Param('id', ParseIntPipe) id: number, @Request() req: any): Promise<CommonApiResponse<null>> {
    // Check if user is admin
    const userRole = req.user?.role;
    if (userRole !== 'admin') {
      throw new HttpException('관리자 권한이 필요합니다', HttpStatus.FORBIDDEN);
    ...
    return CommonApiResponse.success(null, '불량 유형이 성공적으로 삭제되었습니다.');
  }
} 