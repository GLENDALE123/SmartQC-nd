...
export class SharedFolderService {
  ...
  validateImageFile(file: Express.Multer.File): void {
    ...
    if (!this.allowedTypes.includes(file.mimetype)) {
      throw new Error(
        `지원하지 않는 이미지 형식입니다. 허용된 형식: ${this.allowedTypes.join(', ')}`,
      );
    }
  ...
  async uploadImage(
    ...
    try {
      ...
      this.logger.log(
        `✅ 이미지 업로드 성공: ${result.fileName} (DB 저장 완료)`,
      );
      return result;
    } catch (error) {
  ...
  async uploadImageWithInspectionId(
    ...
    try {
      ...
      this.logger.log(
        `✅ 이미지 업로드 성공 (${inspectionType}): ${result.fileName} (검사 ID: ${inspectionId})`,
      );
      return result;
    } catch (error) {
      this.logger.error('❌ 이미지 업로드 실패:', error.message);
  ...
  async deleteImage(
    ...
    try {
      ...
      const originalPath =
        ...
        join(attachment.url, 'original', `${baseFileName}.jpg`);
      const thumbnailPath =
        imageData?.thumbnailPath ||
        join(attachment.url, 'thumbnail', `${baseFileName}-thumb.jpg`);
      const modalPath =
        imageData?.modalPath ||
        join(attachment.url, 'modal', `${baseFileName}-modal.jpg`);
      ...
      for (const file of filesToDelete) {
        try {
          if (existsSync(file.path)) {
            ...
            this.logger.warn(
              `⚠️ ${file.type} 이미지 파일이 존재하지 않음: ${file.path}`,
            );
          }
        } catch (error) {
          this.logger.error(
            `❌ ${file.type} 이미지 파일 삭제 실패: ${file.path}`,
            error.message,
          );
          // 개별 파일 삭제 실패해도 계속 진행
        }
  ...
  async getImagesByInspection(inspectionId: number): Promise<
    Array<{
      id: string;
      fileName: string;
      originalPath: string;
    ...
    try {
      ...
      for (const attachment of attachments) {
        ...
        const originalPath = join(
          attachment.url,
          'original',
          `${attachment.fileName.replace(/\.[^/.]+$/, '')}.jpg`,
        );
        const thumbnailPath = join(
          attachment.url,
          'thumbnail',
          `${attachment.fileName.replace(/\.[^/.]+$/, '')}-thumb.jpg`,
        );
        const modalPath = join(
          attachment.url,
          'modal',
          `${attachment.fileName.replace(/\.[^/.]+$/, '')}-modal.jpg`,
        );
